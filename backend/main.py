...
# ===============================
# OpenWeather & Open-Meteo API Config
# ===============================
OPENWEATHER_API_KEY = os.environ.get("OPENWEATHER_API_KEY", "")
CITY = "Coimbatore"
UNITS = "metric"
LATITUDE = float(os.environ.get("WEATHER_LATITUDE", 22.0))
LONGITUDE = float(os.environ.get("WEATHER_LONGITUDE", 79.0))
...
def decode_wmo_code(code):
    return WMO_CODE_MAP.get(int(code), ("Partly cloudy", "02d"))

# ===============================
# Agmarknet API Config
# ===============================
MAGMARKNET_API_KEY = os.environ.get("MAGMARKNET_API_KEY", "")
...
def get_weather_forecast():
    try:
        url = (
            ...
            f"daily=weather_code,temperature_2m_max,temperature_2m_min,apparent_temperature_max,apparent_temperature_min,uv_index_max&"
        ...
        if response.status_code == 200 and "daily" in data:
            ...
            for i in range(min(5, len(dates))):
                cond_text, icon_code = decode_wmo_code(codes[i] if i < len(codes) else 0)
                max_t = max_temps[i] if i < len(max_temps) else 28.0
                min_t = min_temps[i] if i < len(min_temps) else 22.0
    ...
    # Dynamic fallback starting from today
    from datetime import datetime, timedelta
    today = datetime.now()
    fallback_conditions = [("Moderate Drizzle", "09d"), ("Light Drizzle", "09d"), ("Light Drizzle", "09d"), ("Thunderstorm", "11d"), ("Dense Drizzle", "09d")]
    fallback = []
    for i in range(5):
        d_str = (today + timedelta(days=i)).strftime("%Y-%m-%d")
        cond, icon = fallback_conditions[i % len(fallback_conditions)]
        fallback.append({
            "date": d_str,
            "temp": 29.0 - (i % 2),
            "temp_max": 29.0 + (i % 2),
...
def get_weather():
    try:
        ...
        if response.status_code == 200:
            ...
            cond_text, _ = decode_wmo_code(curr.get("weather_code", 0))
            return {
