...
class YieldForecastingService:
    ...
    def load_model(self):
        try:
            alt_model_path = os.path.join(self.base_dir, "yield_model.pkl")
            alt_columns_path = os.path.join(self.base_dir, "yield_model_columns.pkl")
            ...
            elif os.path.exists(alt_model_path) and os.path.exists(alt_columns_path):
                self.model = joblib.load(alt_model_path)
                self.model_columns = joblib.load(alt_columns_path)
                logger.info("Yield RF Model loaded successfully from yield_model.pkl.")
            else:
